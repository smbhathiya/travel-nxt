...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!locationName || !locatedCity || !locationType) {
      console.log(
        "🔔 [Bookmarks API] missing text fields, attempting to resolve from locationId:",
        locationId
      );
      if (locationId) {
        ...
        if (!loc) {
          return NextResponse.json(
            { error: "Location not found for provided locationId" },
            { status: 404 }
          );
        }
    ...
    if (!locationName || !locatedCity || !locationType) {
      console.warn("🔔 [Bookmarks API] final validation failed - fields:", {
        locationId,
        locationName,
        locatedCity,
        locationType,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (id) whereClause.id = id;
      ...
      whereClause.locationName = locationName;
      whereClause.locatedCity = locatedCity;
    }
