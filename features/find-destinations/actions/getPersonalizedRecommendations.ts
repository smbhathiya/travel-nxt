...
// Fallback function that uses existing user interests if prediction fails
export async function getFallbackRecommendations(): Promise<PersonalizedRecommendation[]> {
  try {
    const { userId } = await auth();
    ...
    console.log('🔄 [Fallback] Using user interests:', user.interests);
    ...
    console.log('🔄 [Fallback] Found locations:', recommendedLocations.length);
    ...
    console.error('❌ [Fallback] Error getting fallback recommendations:', error);
